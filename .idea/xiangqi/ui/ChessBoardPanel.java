...
public class ChessBoardPanel extends JPanel {
    ...
    private static final int SELECTED_BOX_PADDING = 6; // 选中框与棋子的间距
    ...
    private void drawPieces(Graphics2D g) {
        ...
        for (AbstractPiece piece : model.getPieces()) {
            ...
            // 1. 绘制棋子底色（红/黑）
            g.setColor(piece.isRed() ? new Color(200, 0, 0) : Color.BLACK); // 深红更醒目
            ...
            g.setColor(Color.WHITE); // 白字在红/黑底上更清晰
            // 文字居中计算（关键：避免文字偏移出棋子）
            FontMetrics fm = g.getFontMetrics();
            ...
            if (piece instanceof GeneralPiece && model.isInCheck()) {
                boolean isCheckedGeneral = (piece.isRed() == model.isRedTurn());
                if (isCheckedGeneral) {
